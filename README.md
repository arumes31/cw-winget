...
* **Direct PowerShell Action Execution**: User-scoped scheduled tasks execute PowerShell directly using `-WindowStyle Hidden` instead of utilizing a VBScript launcher (`wscript.exe`). This prevents execution blocks in hardened corporate environments where Windows Script Host or `.vbs` scripts are disabled by group policies or Attack Surface Reduction (ASR) rules.
