...
* **Zero Console Window Flashes (VBScript Launcher & Win32 Self-Hiding)**: User-scoped scheduled tasks are executed using `wscript.exe` running a dynamically generated temporary VBScript launcher that invokes the update shell hidden. Windows allocates no console window for `wscript.exe` (being a GUI subsystem host), and the VBScript launcher runs the child PowerShell process completely hidden. As a bulletproof fail-safe, the interactive PowerShell scripts also import the Win32 `ShowWindow` API at startup to instantly hide their own console window in case they are invoked in a visible context by the management agent, ensuring absolutely zero console window flashes, pops, or visible windows on the active user session.
